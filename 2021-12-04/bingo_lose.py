...
print("Length of random number list:", len(random_numbers))
print("Number of boards:", len(bingo_boards))

# Run through random numbers, stopping at the last winner
...
for n in random_numbers:
    ...
    for b, c in zip(bingo_boards, chip_boards):
        if is_winner(c):
            last_winner_score = winning_score(b, c)
            last_random_number = n
        else:
            culled_bingo_boards.append(b)
            culled_chip_boards.append(c)
    bingo_boards = culled_bingo_boards
    chip_boards = culled_chip_boards
    if len(chip_boards) == 0:
        break
print("Final number:", last_random_number)
print("Last winning board score:", last_winner_score)
print("Number * loser board score:", last_random_number * last_winner_score)
