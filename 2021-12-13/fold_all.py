...
def do_fold_x(dots, crease):
    dots_x = dots[:,0]
    assert not np.any(dots_x == crease)
...
def do_fold_y(dots, crease):
    dots_y = dots[:,1]
    assert not np.any(dots_y == crease)
...
def display_dots(dots):
    dots_shape = infer_paper_shape(dots)
    display = np.zeros(dots_shape, dtype=int)
    for d in dots:
        display[d[0],d[1]] = 1
    disp_char = {0:' ', 1:'#'}
    for row in np.transpose(display):
        print("".join([disp_char[n] for n in row]))
